...
- 「ゼロから作るショアのアルゴリズム」シリーズを新たに開始しました(現時点で `320`〜`322`、`324`: 量子加算器、剰余加算器、制御剰余乗算、べき剰余)。既存の理論解説中心の `310_shor.ipynb` の関連編として、算術演算の部品から実際に動く実装を積み上げていきます。べき剰余ができたことで、ショアのアルゴリズムが必要とする量子演算の部品がすべて揃いました — 周囲の位相推定と古典的な後処理は、すでに `310_shor.ipynb` で詳しく解説されています。
