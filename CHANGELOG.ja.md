...
- 「ゼロから作るショアのアルゴリズム」シリーズを新たに開始しました(現時点で `320`〜`322`: 量子加算器、剰余加算器、制御剰余乗算)。既存の理論解説中心の `310_shor.ipynb` の関連編として、算術演算の部品から実際に動く実装を積み上げていきます。元となる記事シリーズの続きに合わせて、今後も追加していく予定です。
